...
Kmeans::InitCenters ()
{
  ...
  while (uniqueMap.size () < (long unsigned int) _K)
    {
...
void
Kmeans::InitPoints (std::list < Point > &pointlist)
{
...
#pragma omp parallel for
  ...
  for (long unsigned int i = 0; i < center.size (); i++)
    {
...
void
Kmeans::SaveEPS (std::string fileName)
{
  std::ofstream fs;
  ...
  if (fs.is_open ())
	...
	{
	  if (max_x < pointsIter->_x)
	    max_x = pointsIter->_x;
	  if (min_x > pointsIter->_x)
	    min_x = pointsIter->_x;
	  if (max_y < pointsIter->_y)
	    max_y = pointsIter->_y;
	  if (min_y > pointsIter->_y)
	    min_y = pointsIter->_y;
	}
      scale = W / (max_x - min_x);
      ...
      for (int k = 0; k < _K; k++, centersIter++)
	{
	  ...
	  for (int j = 0; j < _PointNumber; j++, pointsIter++)
		...
		continue;
	      fs << (pointsIter->_x - cx) * scale +
		W / 2 << " " << (pointsIter->_y - cy) * scale +
		H / 2 << " c\n";

	    }
	  fs << "\n0 setgray " << (centersIter->_x - cx) * scale +
	    W / 2 << " " << (centersIter->_y - cy) * scale + H / 2 << " s\n";
	}
      fs << "\n%%%%EOF";
      delete[]colors;
...
void
Kmeans::SaveSVG (std::string fileName)
{
  int W = 1000;
  int H = 1000;
  ...
  for (int k = 0; k < _K; k++, centerIter++)
      ...
      for (int j = 0; j < _PointNumber; j++, pointsIter++)
	{
	  if (pointsIter->_group != k)
	    continue;
	  doc << svg::Circle (svg::
			      Point ((pointsIter->_x - cx) * scale + W / 2,
				     (pointsIter->_y - cy) * scale + H / 2),
			      1, svg::Fill (svg::Color::White),
			      svg::Stroke (1, c));
	}
      doc << svg::Rectangle (svg::
			     Point ((centerIter->_x - cx) * scale + W / 2,
				    (centerIter->_y - cy) * scale + H / 2), 5,
			     5, svg::Fill (svg::Color::White), svg::Stroke (2,
									    c));
    }
...
void
Kmeans::PrintPointLis (std::list < Point > &pointList)
{
...
void
Kmeans::InitPoints (std::string fileName)
{
  std::ifstream fs;
